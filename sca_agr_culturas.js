/**
 * SCA – Culturas/Produtos da Produção Agrícola v1.0
 * ─────────────────────────────────────────────────────────────────────────────
 * PROBLEMA:
 ...
 *   listas pré-definidas (algumas agrupadas em optgroup). A última opção de
 *   cada lista é "Outros", que libera um campo de texto livre para digitar
 *   um nome que não esteja na lista.
 *
 ...
 *        substitui por um <select> + campo "Outros", preservando o mesmo
 *        data-field (para o restante do sca_core.js continuar lendo/
 *        gravando o valor sem nenhuma alteração).
 *
...
(function () {
  ...
  // ── Substitui o <input data-field="X"> de um item por <select> + "Outros" ─
  function transformarCampo(item, key, field) {
    ...
    // Monta a linha do campo "Outros" (mesmo padrão do sca_pec_racas.js)
    const outroRow = document.createElement('div');
    outroRow.className = 'form-row';
    outroRow.setAttribute('data-agr-outro-row', field);
    outroRow.style.display = 'none';
    const outroLabel = document.createElement('span');
    outroLabel.className = 'form-label';
    outroLabel.style.minWidth = (formRow?.querySelector('.form-label')?.style.minWidth) || '170px';
    const outroInput = document.createElement('input');
    outroInput.className = 'form-input';
    outroInput.setAttribute('data-agr-outro', field);
    outroInput.setAttribute('placeholder', placeholder || 'Digite aqui...');
    outroRow.appendChild(outroLabel);
    outroRow.appendChild(outroInput);

    // Substitui o input antigo pelo select
    input.replaceWith(select);
    // Insere a linha "Outros" logo após a linha do select
    formRow.insertAdjacentElement('afterend', outroRow);
    ...
    function dispararCalc() {
      ...
      // input original (agrBindCalc). Como agora é um <select>, disparamos
      // 'input' manualmente para acionar o mesmo listener (delegado por tipo
      // de elemento não importa, pois agrCalcItem só lê o valor pelo data-field).
    ...
    outroInput.addEventListener('input', function () {
      // Enquanto "Outros" estiver ativo, o valor real do campo passa a ser
      // o texto digitado aqui. Usamos um input oculto auxiliar para não
      // perder o valor quando o select.value permanece "Outros".
      select.setAttribute('data-outro-valor', outroInput.value);
      dispararCalc();
    });

    // Sobrescreve a leitura do valor: quando for "Outros", agrColetarItens()
    // (sca_core.js) lê select.value diretamente via [data-field]. Para que
    // o valor salvo seja o texto digitado (e não a palavra "Outros"),
    // mantemos o <select> com um getter customizado via Object.defineProperty
    // não é viável em <select> nativo — então interceptamos no momento da
    // coleta, abaixo, via MutationObserver-free approach: ver hookColeta().
  }

  // ── Garante que o valor salvo seja o texto livre quando "Outros" está ativo
  //    Intercepta agrColetarItens (chamada por salvarAgricola) para que,
  //    nos campos-alvo com "Outros" selecionado, o valor gravado seja o
  //    texto digitado no campo livre, e não a string "Outros".
  function hookColeta() {
    if (typeof window.agrColetarItens !== 'function' || window.agrColetarItens.__patchedCulturas) return false;
    const original = window.agrColetarItens;
    window.agrColetarItens = function (key) {
      const itens = original.apply(this, arguments);
      const field = AGR_CAMPO_ALVO[key];
      if (!field) return itens;
      const lista = document.getElementById('agr-' + key + '-lista');
      if (!lista) return itens;
      const els = lista.querySelectorAll('.agr-item');
      els.forEach((item, i) => {
        const select = item.querySelector(`select[data-field="${field}"][data-agr-select]`);
        if (select && select.value === 'Outros') {
          const outroInput = item.querySelector(`[data-agr-outro="${field}"]`);
          if (outroInput && itens[i]) itens[i][field] = outroInput.value || '';
        }
      });
      return itens;
    };
    window.agrColetarItens.__patchedCulturas = true;
    return true;
  }
