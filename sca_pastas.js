...
const PASTA_PREFIXO  = 'empresa01';       // Prefixo isolado para esta empresa
...
window.scaLerArquivosDaPasta = async function () {
  ...
  for (const item of itens) {
    ...
    try {
      ...
      if (item.name.endsWith('.enc')) {
        const dec = await _scaDecrypt(await blob.arrayBuffer());
      ...
      console.warn('[SCA IA] Falha ao ler arquivo da pasta:', item.name, e);
    }
  }
  return arquivos;
};
