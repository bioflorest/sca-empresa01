...
(function () {
  ...
  const CSS = `
    ...
    .lic-logo-img { width:220px;height:auto;object-fit:contain;margin:0 auto 6px;display:block; }
    .lic-logo {
      font-family: 'Rajdhani', sans-serif;
      font-size: 2rem; font-weight: 700;
      color: #00e676; letter-spacing: 4px;
      margin-bottom: 2px;
    }
    .lic-sub {
    ...
    .lic-barra-fundo { background:#1a2a1a; border-radius:999px; height:5px; overflow:hidden; }
    .lic-barra-fill  { height:100%; border-radius:999px; transition:width 1.2s ease; }
    .lic-btn-ok {
      width:100%; padding:.85rem;
      background: linear-gradient(135deg,#27ae60,#00e676);
    ...
    .lic-bloqueado p  { color:rgba(255,255,255,.45); font-size:.85rem; line-height:1.6; margin-bottom:1.4rem; }
    .lic-btn-renovar {
      display:inline-block; padding:.8rem 2rem;
      background:#e63946; color:#fff; border:none; border-radius:9px;
      font-family:'Rajdhani',sans-serif; font-size:1rem; font-weight:700;
      cursor:pointer; letter-spacing:1px; text-decoration:none;
      transition:opacity .2s;
    }
    .lic-btn-renovar:hover { opacity:.85; }
  `;
  ...
  function telaBloqueada(msg) {
    mostrarOverlay(`
      <img class="lic-logo-img" src="https://hlkakdaulmenrcyarhgq.supabase.co/storage/v1/object/public/icones-sistema/SCA.png"/>
      <div class="lic-logo">SCA</div>
      <div class="lic-sub">Sistema de Consultoria Ambiental</div>
  ...
  function telaAviso(lic, diasRest, status, venc, duracaoTotal) {
    ...
    mostrarOverlay(`
      ...
      <div class="lic-info">
        ...
        <div class="lic-row"><span class="lk">Plano</span><span class="lv">${lic.plano === 'anual' ? 'Anual' : 'Mensal'}</span></div>
  ...
  async function verificarLicenca() {
    ...
    try {
      ...
      // Sempre verifica a licença do admin principal da empresa
      // Se o admin estiver bloqueado, todos os usuários são bloqueados
      const { data, error } = await window.supa
        .from('licencas')
        .select('chave, cliente, plano, vencimento, ativo')
        .eq('email', 'admin@empresa01.com.br')
        .single();
      ...
      const duracaoTotal = data.plano === 'anual' ? 365 : 30;
